...
---
## 개인 정리
- 괄호는 무조건 스택이 개꿀이다. -> 괄호의 특성상 stack의 ADT만 구성할 경우 괄호의 유효성이 확인 가능하다.<br>
        -Why?
          - 만약 [{()}]을 생각해보면 쉽다. 만약 대괄호 안에서 중괄호나 소괄호가 시작한다면, ) 나 }는 ]안에 있어야 한다. (일반론적으로) 그렇지 않다면 잘못된 것이다. 그렇다면 당연히 스택이 편할 수 밖에 없다는 것이다!!
---
